...
export default function SchedulingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingConfig) {
        await schedulingService.updateConfig(editingConfig.id, data);
      } else {
        await schedulingService.createConfig(data);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await schedulingService.deleteConfig(id);
  ...
  const handleToggleEnabled = async (config) => {
    try {
      await schedulingService.updateConfig(config.id, {
  ...
  const handleTrigger = async (configId) => {
    ...
    try {
      await schedulingService.triggerGeneration(configId);
        ...
        <div>
          ...
          >
            <div>
              ...
              >
                Automate article generation on a daily schedule
              </p>
            </div>
            {!showForm && (
              <button
                      ...
                      {logs.map((log) => (
                        ...
                        >
                          ...
                          <td style={{ padding: "0.75rem" }}>
                            {log.article_id ? (
                              <Link
                                href={`/dashboard/articles/${log.article_id}`}
                              >
                                <span
                                  style={{
                                    color: "var(--primary-color)",
